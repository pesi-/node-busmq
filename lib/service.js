 ...
 */
Service.prototype.serve = function(options) {
  if (this.isServing()) {
 ...
 */
Service.prototype.request = function(message, cb) {
  if (!this.isConnected()) {
    return;
  }
  ...
  if (cb) {
    ...
    this.pendingReplies[reqId] = cb;
  }
  return this.qService.push({data: message, replyTo: replyTo, reqId: reqId});
};
...
Service.prototype._handleReply = function(reply, id) {
  ...
  try {
    ...
    var cb = this.pendingReplies[reply.reqId];
    if (!cb) {
      return;
    }
    // invoke the callback
    cb.call(null, reply.err, reply.reply);
