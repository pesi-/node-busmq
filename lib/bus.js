 ...
 * @param {*} object one of queue, channel, pubsub, service
 */
Bus.prototype.promisify = function(object) {
  if (!util.promisify) {
    throw new Error(`promisify is not supported in node ${process.version}`);
  }

  if (!object._promisifiable) {
    throw new Error(`promisify is not supported for this object`);
  }

  object._promisifiable().forEach(function(method) {
