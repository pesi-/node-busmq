...
var redisUrls2 = ['redis://127.0.0.1:8000', 'redis://127.0.0.1:8001', 'redis://127.0.0.1:8002'];

describe('BusMQ Cluster', function() {

  var redisCluster = new RedisCluster(7000);
  var redisCluster2 = new RedisCluster(8000);
  ...
  before(function(done) {
    redisCluster.start(function() {
      redisCluster2.start(done);
    });
  });

  // stop all redis servers
  after(function(done) {
    redisCluster2.stop(function() {
  ...
  describe('federation', function() {
    ...
    it('federation websocket of queue closes and reopens', function(done) {
      ...
      fedserver.listen(9788, function() {
        var bus = Bus.create({driver: 'ioredis', layout: 'cluster', redis: redisUrls, federate: {server: fedserver, path: '/federate'}, logger: console});
        tf.fedWebsocketQueueClosesReopens(bus, fedBusCreator, fedserver, 9788, function(err, fedserver) {
          fedserver.close();
          setTimeout(function() {
            done(err);
          }, 100);
    ...
    it('federation websocket of channel closes and reopens', function(done) {
      ...
      fedserver.listen(9789, function() {
        var bus = Bus.create({driver: 'ioredis', layout: 'cluster', redis: redisUrls, federate: {server: fedserver, path: '/federate'}, logger: console});
        tf.fedWebsocketChannelClosesReopens(bus, fedBusCreator, fedserver, 9789, function(err, fedserver) {
          fedserver.close();
          setTimeout(function() {
            done(err);
          }, 100);
    ...
    it('allows federation with custom secret function', function(done) {
      var custom = {};
      var bus = Bus.create({driver: 'ioredis', layout: 'cluster', redis: redisUrls, federate: {server: fedserver, secret: function(info) {
        custom.called = true;
